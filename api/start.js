...
import { Client, GatewayIntentBits, REST, Routes, SlashCommandBuilder } from 'discord.js';
import { get, set } from '@vercel/edge-config';  // Assume set exists; if not, replace with REST API call

const clients = new Map(); // token => client instance (cached)
const blockedIPs = new Map(); // ip => unblock timestamp (ms) (cached)

const COOLDOWN_MS = 30 * 60 * 1000; // 30 minutes

// Helper to hydrate caches from Edge Config on each request
async function hydrateCaches() {
    try {
        const storedBlockedIPs = await get('blockedIPs');
        if (storedBlockedIPs && typeof storedBlockedIPs === 'object') {
            for (const [ip, time] of Object.entries(storedBlockedIPs)) {
                blockedIPs.set(ip, time);
            }
        }
        const storedClients = await get('clients');
        if (storedClients && typeof storedClients === 'object') {
            // We cannot store Client instances, so just track active tokens:
            for (const token of Object.keys(storedClients)) {
                clients.set(token, null); // placeholder for client instance
            }
        }
    } catch (err) {
        console.error('Failed to hydrate caches from Edge Config:', err);
    }
}

// Helper to persist caches to Edge Config after updates
async function persistCaches() {
    try {
        // Save blockedIPs as simple object
        const blockedIPsObj = {};
        for (const [ip, time] of blockedIPs.entries()) {
            blockedIPsObj[ip] = time;
        }
        await set('blockedIPs', blockedIPsObj);

        // Save clients tokens (presence only, no instances)
        const clientsObj = {};
        for (const token of clients.keys()) {
            clientsObj[token] = true;
        }
        await set('clients', clientsObj);
    } catch (err) {
        console.error('Failed to persist caches to Edge Config:', err);
    }
}
...
export default async function handler(req, res) {
    ...
    if (clients.has(token) && clients.get(token)?.isReady && clients.get(token).isReady()) {
        return res.status(200).send('Bot already running for this token.');
    }
    ...
    try {
        ...
        setTimeout(() => {
            client.destroy();
            clients.delete(token);
            persistCaches();  // best effort async, no await here
            console.log(`⌛ Bot stopped for token ${token.substring(0, 8)}...`);
        }, 300000);
